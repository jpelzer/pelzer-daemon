...
public class PIDWatcherDaemon{
  ...
  public static void main(final String[] args){
    ...
    new PanicHelper();
    while(true){
      log.info("PIDWatcherThread is alive.");
      new PIDWatcherDaemon().scanFolder(Constants.ROOT_FOLDER);
      try{
        Thread.sleep(60000);
  ...
  private void scanFolder(final String rootFolder){
    ...
    if(!folder.exists() || !folder.isDirectory() || folder.listFiles() == null){
      log.error("Can't scan '" + rootFolder + "', it either doesn't exist or it isn't a folder we can see.");
      return;
    }
    ...
    for(final File file : files)
      ...
      try{
        ...
        if(!PID.isPIDAlive(pid)){
          log.error("'" + file.getPath() + "' (" + pid + ") is not alive.");
        ...
        log.error("IOException while reading pid '" + file.getPath() + "'", ex);
      }
  }
  ...
  private boolean sendPanic(final String messageStr){
    return PanicHelper.sendPanic("PIDWatcherDaemon", messageStr);
  }

  public static class Constants extends OverridableFields{
    public static String ROOT_FOLDER;
    // ~two gigabytes.
    public static long LOG_SIZE_HIGH_WATER_MARK = 2 * 1000 * 1000 * 1000;
